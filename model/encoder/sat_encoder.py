...
class Conv3d_SAT(nn.Module):
    def __init__(self, in_channels, 
                 ...
                 nlayers, 
                 dropout):
        super(Conv3d_SAT, self).__init__()
        # 初始化一个空列表来存储Conv1d层
        layers = []
        ...
        self.fc1 = nn.Linear(channel_num_list[-1]*22*22, tf_input_dim)
...
if __name__=='__main__':
    ...
    # # 实例化模型并训练
    model = Conv3d_SAT(in_channels = 4, 
                    ...
                    nlayers=6, 
                    dropout=0.1)
    sat_data = np.expand_dims(sat_data, axis=0)
    sat_data = torch.from_numpy(sat_data).float() 
    print(model(sat_data[:,:,0:4,:,:]))
