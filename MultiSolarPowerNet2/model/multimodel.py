...
class MyRelu(nn.Module):
    ...
    def forward(self, x):
        return torch.where(x >= 15, x, torch.tensor(0.0, device=x.device))
...
class MultiModel(BaseModel):
    def __init__(self, config ):
        ...
        if config.include_nwp:
            self.conv3d_nwp_list = nn.ModuleList()
        ...
        if config.include_sat:
            self.conv3d_sat_list = nn.ModuleList()
            for i in range(config.sat_source_num):
                conv3d_sat = Conv3d_SAT(in_channels=config.sat_in_channels[i],  
                                    kernel_sizes=config.sat_kernel_sizes[i],
                                    paddings=config.sat_paddings[i], 
                                    channel_num_list = config.sat_channel_num_list[i], 
                                    tf_input_dim = config.sat_tf_input_dim[i],
                                    nhead=config.sat_nhead[i], 
                                    nhid=config.nwp_nhid[i], 
                                    nlayers=config.nwp_nlayers[i], 
                                    dropout=0.3)
                self.conv3d_sat_list.append(conv3d_sat)        
